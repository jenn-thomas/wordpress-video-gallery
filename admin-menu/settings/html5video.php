...
define( 'WIDTH', $argv[7] );
define( 'HEIGHT', $argv[8] );
define( 'FFMPEG_PATH', $argv[9] );
...
$sql = "SELECT * FROM " . WP_DB_PREFIX . "nev_videos ORDER BY ID DESC LIMIT 0 , 1;";
$video_data = $mysqli->query($sql)->fetch_object();
...
if( file_exists($video_encoded_name . '.mp4' ) ){
    ...
    echo 'Error: converted video file not found! Path: ' . $video_encoded_name . '.mp4';
}

if (isset($video_data)){
	$image = $video_encoded_name . '.png';
	$encoded_video = $video_encoded_name . '.mp4';
	$mysqli->query("UPDATE " . WP_DB_PREFIX . "nev_videos SET videofile='" . $encoded_video . "' WHERE id=" . $video_data->id . ";");
	$mysqli->query("UPDATE " . WP_DB_PREFIX . "nev_videos SET thumbnail='" . $image . "' WHERE id=" . $video_data->id . ";");
	$mysqli->query("UPDATE " . WP_DB_PREFIX . "nev_videos SET converted=1 WHERE id=" . $video_data->id . ";");
	unlink( VID_FILE );
}
